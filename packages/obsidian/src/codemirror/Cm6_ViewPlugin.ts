			...
			private syncMonacoBlock(monaco: any, block: LivePreviewCodeBlock, lineElements: HTMLElement[]): void {
				...
				if (!handle) {
					...
					const editor = monaco.editor.create(container, {
						...
						lineHeight,
						lineNumbers: plugin.loadedSettings.ecDefaultShowLineNumbers ? 'on' : 'off',
						lineNumbersMinChars: plugin.loadedSettings.ecDefaultShowLineNumbers ? 4 : 0,
						...
						folding: false,
						glyphMargin: false,
						lineDecorationsWidth: plugin.loadedSettings.ecDefaultShowLineNumbers ? 8 : 0,
